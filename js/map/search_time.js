        ...
        if ($('.time-select-second-change-img').attr('src') == '../images/magnifier.png') {
            ...
            */
            ...
            if($('.time-select-past-time').hasClass('on-check')) {//假如现在处于过去标签，则请求查看流量和使用率
                showFlowChange(timeStart, timeEnd, lng, lat);
                showUserAtio(timeStart, timeEnd, lng, lat);
            } else {//假如现在处于未来标签，即请求未来的数据，调用预测的函数
                estimationFlowChange(timeStart, timeEnd, lng, lat);
                estimationUserAtio(timeStart, timeEnd, lng, lat);
            }
